...
module.exports = function (program) {
  ...
  sched.command('list <workspaceId>').alias('ls').description('List schedules for a workspace')
    .addHelpText('after', '\nExamples:\n  $ citrus schedules list 3\n  $ citrus sched ls 3 --json')
    .action(withErrors(async (wsId, opts, cmd) => {
      await setup(cmd);
      const data = await client.get(`/api/schedules/${wsId}`);
      const rows = Array.isArray(data) ? data : data.schedules || [];
      output.table(rows, COLS, { title: `Schedules (Workspace #${wsId})`, emptyMessage: 'No schedules found.' });
    }));
