...
Examples:
  ...
  flows
    ...
    .addHelpText('after', `
Examples:
  $ citrus flows import flow.json -w <workspaceId>
  $ citrus flows import ./exported-flow.json -w <workspaceId> --json`)
    .action(withErrors(async (file, opts, cmd) => {
      await setup(cmd);
      const content = JSON.parse(fs.readFileSync(file, 'utf8'));
      const payload = { workspace_id: opts.workspace, flow_data: content };
      const data = await client.post('/api/flow-versions/import', payload);
      const record = data.flow || data;
      output.success(`Imported flow "${record.name || 'flow'}" (ID: ${record.id || 'n/a'}) into workspace #${opts.workspace}`);
      if (output.isJson) output.json(record);
    }));
