  ...
  $ citrus flows ls --json`)
    .action(withErrors(async (opts, cmd) => {
      ...
      const url = opts.workspace
        ? `/api/flows/workspace/${opts.workspace}`
        : '/api/flows';
...
Examples:
  ...
  $ citrus flows create -n "Test" -w 1 --definition '{"nodes":[],"edges":[]}'`)
    .action(withErrors(async (opts, cmd) => {
      ...
      if (opts.fromFile) {
        body.definition = JSON.parse(fs.readFileSync(opts.fromFile, 'utf8'));
      } else if (opts.definition) {
        body.definition = JSON.parse(opts.definition);
      }
...
Examples:
  ...
  code
    ...
    .description('Update flow code from a JSON file')
    .addHelpText('after', `
Examples:
