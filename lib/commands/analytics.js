...
module.exports = function (program) {
  ...
  analytics.command('activity').description('Recent activity metrics')
    .action(withErrors(async (opts, cmd) => {
      ...
      const d = data.activity || data;
      Object.entries(d).forEach(([k, v]) => {
        const label = k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        output.log(`  ${label}: ${typeof v === 'object' ? JSON.stringify(v) : v}`);
      });
      output.log();
    }));
    ...
    .action(withErrors(async (opts, cmd) => {
      ...
      if (typeof d === 'object' && !Array.isArray(d)) {
        ...
        output.table(d, [
          ...
          { key: 'email', label: 'Email', width: 25, transform: maskEmail },
          { key: 'role', label: 'Role', width: 12 },
          { key: 'status', label: 'Status', width: 10, transform: v => output.status(v) }
        ], { title: 'Users' });
      }
      output.log();
    }));
