...
Examples:
  ...
  $ citrus deploy ls --json`)
    .action(withErrors(async (opts, cmd) => {
      ...
      const rows = Array.isArray(data) ? data : data.deployments || [];
      output.table(rows, COLS, { title: 'Deployments', emptyMessage: 'No deployments found.' });
    }));
  ...
  // ─── Dispatch ───────────────────────────────────────────────
  deploy.command('dispatch <id>').description('Dispatch a deployment event')
    ...
    .action(withErrors(async (id, opts, cmd) => {
      ...
      output.success(`Dispatched event to deployment #${id}`);
      if (output.isJson) output.json(data);
    }));

  // ─── Late / Clear-Late ──────────────────────────────────────
  deploy.command('late <id>').description('Mark deployment as late')
    .action(withErrors(async (id, opts, cmd) => {
      await setup(cmd);
      await client.post(`/api/runtime/deployments/${id}/late`);
      output.success(`Marked deployment #${id} as late`);
    }));

  deploy.command('clear-late <id>').description('Clear late status')
    .action(withErrors(async (id, opts, cmd) => {
      await setup(cmd);
      await client.post(`/api/runtime/deployments/${id}/clear-late`);
      output.success(`Cleared late status for deployment #${id}`);
    }));

  // ─── Set Count ──────────────────────────────────────────────
  deploy.command('set-count <id>').description('Set deployment count')
    .requiredOption('-c, --count <n>', 'Count value')
    .action(withErrors(async (id, opts, cmd) => {
      await setup(cmd);
      await client.post(`/api/runtime/deployments/${id}/set-count`, { count: parseInt(opts.count) });
      output.success(`Set count to ${opts.count} for deployment #${id}`);
    }));
      ...
      for (let cycle = 1; cycle <= maxCycles; cycle++) {
        ...
        for (const route of routes) {
          ...
          for (let count = 1; count <= threshold; count++) {
            // Set count via API
            try {
              await client.post(`/api/runtime/deployments/${id}/set-count`, {
                route: route.route,
                count
              });
            } catch (err) {
            ...
            // If cycle wrapped (count reset), break inner loop
            if (routeState && routeState.count === 0 && count > 1) {
              process.stdout.write(colors.green(`  ← CYCLE COMPLETE`));
              break;
            }
          }

          console.log(''); // newline after route progress
        }
