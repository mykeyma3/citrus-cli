...
module.exports = function (program) {
  ...
  pages.command('create <appId>').description('Create a page')
    ...
    .option('--content-file <path>', 'Page content from a JSON file')
    .addHelpText('after', `
Examples:
  $ citrus apps pages create 7 -t "Dashboard"
  $ citrus apps pages create 7 -t "Settings" --content-file page.json

Note: On PowerShell, use --content-file instead of -c for JSON content,
or escape JSON with: -c '{\\"key\\":\\"value\\"}'`)
    .action(withErrors(async (appId, opts, cmd) => {
      await setup(cmd);
      const body = { name: opts.title };
      if (opts.slug) body.slug = opts.slug;
      if (opts.contentFile) {
        const fs = require('fs');
        body.content = JSON.parse(fs.readFileSync(opts.contentFile, 'utf8'));
      } else if (opts.content) {
        body.content = JSON.parse(opts.content);
      }
  ...
  access.command('grant <appId>').description('Grant access')
    ...
    .option('-t, --type <type>', 'Grantee type (user, team)', 'user')
    .option('-r, --role <role>', 'Permission (view, edit, deploy, admin)', 'view')
    .action(withErrors(async (appId, opts, cmd) => {
      await setup(cmd);
      await client.post(`/api/apps/${appId}/access`, {
        grantee_type: opts.type,
        grantee_id: opts.user,
        permission: opts.role
      });
      output.success(`Granted ${opts.role} access to ${opts.type} #${opts.user}`);
    }));
