...
const tracker = require('./ae-tracker');
...
/**
 * Wraps a command action with error handling + Aestimor tracking.
 * Usage: .action(withErrors(async (args, opts, cmd) => { ... }))
 */
function withErrors(fn) {
  return async (...args) => {
    // Fire-and-forget: track every command execution
    try {
      const { command, action } = tracker.extractCommandInfo(args);
      tracker.track(command, action);
    } catch (_) { /* never interfere */ }
